let cardsArray = [
  ...
  // Grab the div with an id of game-board and assign to variable game
  ...
  // Loop thought each item in our cards array
  ...
  grid.addEventListener('click', function(event) {
      ...
      // Only select dics inside the grid
      if (clicked.nodeName === 'SECTION' || clicked === previousTarget || clicked.parentNode.classList.contains('match' || clicked.parentNode.classList.contains('selected'))) {
          return;
      }
      // Only want to add `selected` class if the current count is less than 2
      if (count < 2) {
        count++;
